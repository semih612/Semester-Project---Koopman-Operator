...
simulation_error_perc = zeros(num_lifted_states,Ntraj, Nsim);
simulation_error_box = zeros(num_lifted_states,Ntraj, Nsim);
box = [4*pi,10*pi,2*pi,10*pi, 2, 1, 1, 0.7698, 2];
...
for sim = 1:Nsim
    ...
    for k = 1:Ntraj
        ...
        for j=1:num_lifted_states
            simulation_error_perc(j, k+1,sim) = ((psi_xk1_true(j) - psi_xk_sim(j,k+1,sim))/psi_xk1_true(j)).^2;
        end
        for j=1:num_lifted_states
            simulation_error_box(j, k+1,sim) = ((psi_xk1_true(j) - psi_xk_sim(j,k+1,sim))/box(j)).^2;
            %simulation_error_box(j, k+1,sim) = abs((psi_xk1_true(j) - psi_xk_sim(j,k+1,sim))/box(j));
            %simulation_error_box(j, k+1,sim) = abs((psi_xk1_true(j) - psi_xk_sim(j,k+1,sim))/sum(box));
            %simulation_error_box(j, k+1,sim) = abs((psi_xk1_true(j) - psi_xk_sim(j,k+1,sim))/1);
        end
